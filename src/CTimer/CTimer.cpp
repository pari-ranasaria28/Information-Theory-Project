...
void CTimer::start()
{
    if (isRunning)
    {
        std::cout << "(EE) CTimer :: trying to start a CTimer object that is already running !" << std::endl;
    }
    isRunning = true;
    QueryPerformanceCounter(&t_start);
}
...
void CTimer::stop()
{
    if (!isRunning)
    {
        std::cout << "(EE) CTimer :: trying to stop a CTimer object that is not running !" << std::endl;
    }
...
long CTimer::get_time_ns()
{
    ...
    long long diff = (t_stop.QuadPart - t_start.QuadPart);
    long long ns = (diff * 1000000000LL) / freq.QuadPart;
